// lab.go — the offline verification lane driver (`mediactl lab ...`):
...
// cmdLab dispatches the verification-lane subcommands.
...
func labUsage(w *os.File) {
	fmt.Fprint(w, `use: lab matrix <a|b|e|user_posts> [flags]
     lab audit-comments --store <dir> [flags]
     lab vr-slice --sec-uid <id> [--mock=true]

matrix  run one TESTING.md matrix group offline (fixture-driven mock
        platforms) writing a three-valued judgment report per row to
        adapt/reports/matrix-<group>-<ts>.json
audit-comments
