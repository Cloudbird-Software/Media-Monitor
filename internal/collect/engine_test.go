...
// TestBindFailClosed: a missing binding path and an empty list both error.
func TestBindFailClosed(t *testing.T) {
	responses := []string{`{"other": 1}`, `{"data": []}`}
	for i, body := range responses {
		...
		srv.Close()
		if err == nil {
			t.Fatalf("case %d: SearchItems succeeded, want fail-closed error", i)
		}
		if i == 0 && !strings.Contains(err.Error(), "missing") {
			t.Fatalf("case %d: err = %v, want missing binding error", i, err)
		}
		if i == 1 && !strings.Contains(err.Error(), "empty") {
			t.Fatalf("case %d: err = %v, want empty list error", i, err)
		}
	}
}
