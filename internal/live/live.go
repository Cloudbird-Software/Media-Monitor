...
func (c *Config) runSession(ctx context.Context, roomID string, cursor *string, heartbeat time.Duration, handler func(ev model.LiveEvent) error) error {
	wssURL, err := buildWSSURL(roomID, *cursor, c.Signer)
	if err != nil {
		return err
	}
...
func buildWSSURL(roomID, cursor string, sign SignFn) (string, error) {
	base := wssEndpointRoot() + "/webcast/im/push/v2/?" +
		...
		"&browser_version=5.0&browser_online=true&tz_name=Asia/Shanghai&aid=6383" +
		"&live_id=1&did_rule=3&endpoint=live_pc&support_wrds=1" +
		"&user_unique_id=" + randomUniqueID() +
		"&im_path=/webcast/im/fetch/&identity=audience&need_persist_msg_count=15" +
		"&room_id=" + roomID + "&sub_room_id=&sub_channel_id=&heartbeatDuration=0" +
