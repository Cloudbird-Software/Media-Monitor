...
func TestResolveAndEvents(t *testing.T) {
	...
	if ev.Content != "你好 world" || ev.Time != 1700000001 || ev.RoomID != roomID {
		t.Errorf("chat = %+v", ev)
	}
	u := ev.User
	...
	ev = events[11]
	if ev.Event != "control" || ev.RawSummary["status"] != int64(3) || ev.Content != "bye" || ev.Time != 1700000010 {
...
func TestBuildWSSURL(t *testing.T) {
	...
	cfg := &Config{Registry: testRegistry(t), Signer: sign}
	u, err := cfg.buildWSSURL(roomID, "cur-42")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.HasPrefix(gotQuery, "aid=6383&app_name=douyin_web&") || strings.Contains(gotQuery, "signature=") {
