...
红队第 4 轮修复（R4）：dy 评论确定性派生 + 短 TTL 响应缓存（P2-1）、count=0 → 0 条
  与 page_size 钳制/越界 cursor 收敛（P3-1/2/3）、乱码关键词空态（P3-4）、
  /favicon.ico + /robots.txt 200 与页面 link icon（P3-6/7）、Server 头去尾随空格（P3-8）。
...
（窗口页数自适应：min(400, 数据集页数)），保证「换关键词 → 结果集确实刷新」，同时保持确定性——e2e 用同一
公式即可核对「页面数据 == synthgen 数据集」。
...
def rotation_pages(site: str, keyword: str | None) -> int:
    ...
    数据窗口——确定性不变（模数由 MANIFEST count 唯一确定）。"""
...
# ---------------------------------------------------------------------------
# 短 TTL 响应缓存（红队 R4-P2-1：dy 评论端点 4s 内重复请求逐字节一致）
#
# 渲染层已确定性派生（cid/create_time/reply_comment_total 跨请求恒定），但信封内
# extra.now（请求时点）与 log_pb.impr_id（真站每请求唯一，R2-P3-1 修复项）天然
# 逐请求变化。本缓存对「同一 (端点, id, 游标, count)」的响应 dict 做 8s TTL 内
# 原对象复用（≥4s 窗口），使复读/复核型流程拿到逐字节一致的响应；TTL 过期后
# 重新渲染（确定性内容不变，仅刷新时变字段）。
# ---------------------------------------------------------------------------
_RESP_CACHE: dict = {}
...
# ---------------------------------------------------------------------------
# 乱码/无意义关键词空态（红队 R4-P3-4）
#
# 真站对 gibberish 词（乱码串、单字母）接近 0 结果；合成站此前任何词都映射到
# 满页窗口。判定规则（确定性、无出网）：
#   - 含 ≥2 字连续 CJK 词元 → 有意义（中文搜索的最小语义单位，测试关键词
#     「美食教程/旅行vlog/露营装备」等全部命中）；
#   - 否则含 ≥3 位 latin/数字词元且该词元出现在站点语料词汇表（synthgen 题材池
#     + 数据集文本抽样提取，如 vlog/ootd/citywalk）→ 有意义；
#   - 其余（单字母、纯标点、无意义乱码如 adyj!!~零）→ 空态信封。
# ---------------------------------------------------------------------------
_CJK_RUN_RE = re.compile(r"[\u4e00-\u9fff]{2,}")
_LATIN_RUN_RE = re.compile(r"[A-Za-z][A-Za-z0-9\-]{2,}")
_SITE_VOCAB: dict = {}
_SITE_VOCAB_LOCK = threading.Lock()
_VOCAB_SCAN_RECORDS = 5000  # 数据集文本抽样上限（题材词循环出现，抽样即稳定）


def _site_vocab(site: str) -> set:
    """站点 latin 词汇表：synthgen 题材池 + 数据集文本字段抽样（确定性、懒加载）。"""
    with _SITE_VOCAB_LOCK:
        vocab = _SITE_VOCAB.get(site)
        if vocab is not None:
            return vocab
        vocab = set()
        try:
            pools = json.loads((_ORACLE_ROOT / "synthgen" / "data" / "%s_pools.json" % site)
                               .read_text(encoding="utf-8"))
            texts: list = []
            for lst in (pools.get("topics") or {}).values():
                texts.extend(lst)
            for lst in (pools.get("nicknames") or {}).values():
                texts.extend(lst)
        except Exception:
            texts = []
        state = STATES[site]
        try:
            ds = state.dataset()
            n = min(_VOCAB_SCAN_RECORDS, len(ds))
            for i in range(n):
                r = ds.read(i)
                if site == "douyin":
                    texts.append(r.get("desc") or "")
                elif site == "xhs":
                    texts.append((r.get("note_card") or {}).get("display_title") or "")
                else:
                    texts.append((r.get("photo") or {}).get("caption") or "")
        except Exception:
            pass
        for t in texts:
            for m in _LATIN_RUN_RE.findall(t or ""):
                vocab.add(m.lower())
        _SITE_VOCAB[site] = vocab
        return vocab


def keyword_meaningful(site: str, keyword: str | None) -> bool:
    """搜索关键词是否可能命中内容（False → 走空态信封）。空关键词交由各端点原逻辑。"""
    kw = (keyword or "").strip()
    if not kw:
        return True
    if _CJK_RUN_RE.search(kw):
        return True
    for tok in _LATIN_RUN_RE.findall(kw):
        if tok.lower() in _site_vocab(site):
            return True
    return False
...
def dy_search_stream(state: SiteState, params: dict) -> dict:
    ...
    if offset < 0:  # 红队 P2 修复 10：负 offset 视为 0
        offset = 0
    if not keyword_meaningful("douyin", keyword):  # R4-P3-4：乱码/无意义词 → 空态
...
def xhs_search_notes(state: SiteState, body: dict) -> tuple[dict, dict]:
    ...
    if not keyword_meaningful("xhs", str(keyword)):  # R4-P3-4：乱码/无意义词 → 空态
        return None, synth_render.xhs_empty_search_response()
    page = body.get("page")
    page = 1 if page is None else max(1, int(page))
    size = body.get("page_size")
    size = DEFAULT_PAGE_SIZE if size is None else int(size)
    ...
    return synth_render.render_xhs_search_notes(
        state.dataset(), page, size, body.get("search_id"), keyword, start=start)
...
def ks_photo_comment_list(state: SiteState, body: dict) -> dict:
    ...
    body = dict(body or {})
    photo_id = str(body.get("photoId") or "")
    pcursor = str(body.get("pcursor") or "")
...
def ks_kconf_get(state: SiteState, body: dict) -> dict:
    ...
    body = dict(body or {})
    return synth_render.render_ks_kconf_get(state.dataset(), str(body.get("kconfKey") or ""))
...
class SynthHandler(BaseHTTPRequestHandler):
    ...
    def _send(self, code: int, body: bytes, ctype: str, extra: dict | None = None):
        # 红队 R3-P3-5：客户端带 Accept-Encoding: gzip 时按 gzip 压缩（真站三站均压缩）
        ae = (self.headers.get("Accept-Encoding") or "")
        if "gzip" in ae.lower() and code != 204 and body:
            import gzip as _gz
        ...
        self.send_header("Vary", "Origin")  # ACAO 按 Origin 变化，禁缓存串答
    ...
    def do_OPTIONS(self):
        # 红队 R3-P3-5：OPTIONS 预检补 Allow 头（真站形态）
        self._send(204, b"", "text/plain", {"Allow": "GET, POST, OPTIONS"})
    ...
    def do_GET(self):
        ...
        except Exception as e:  # 诊断友好（对齐 Hoverfly miss 行为的可观测性）
            import traceback
            traceback.print_exc()
    ...
    def _serve_state_page(self, fname: str, state_obj: dict):
        ...
        if not f.is_file():
            return self._json(503, {"error": "page skeleton missing: run build_pages.py (%s)" % f})
    ...
    def _page_alias(self, path: str, params: dict):
        """真站 URL 路径别名（红队 P2-D6/P3-4 + R3-P1-2）+ 详情/作者页 SSR 数据注入。

        - dy  /search/<kw> → search.html；/video/<id> → detail.html（不存在 id 200 + 页面错误态）
              /user/<sec_uid> → profile.html（作者主页）
        - xhs /search_result?keyword= → search.html；/explore/<id> → detail.html；
              不存在 id → 302 /404（真站形态）；/user/profile/<uid> → profile.html
        - ks  /search/video?searchKey= → search.html；/short-video/<id> → detail.html；
              /profile/<id> → profile.html（真站 URL 约定）
        """
        state = STATES[self.site]
        if self.site == "douyin":
            m = _DY_SEARCH_PATH_RE.match(path)
            if m and path != "/search/":  # 裸 /search/ 走默认页
                return self._serve_file("search.html")
            m = _DY_VIDEO_PATH_RE.match(path)
            if m:  # 不存在 id：真站 200 + 页面错误组件（build_pages 详情 JS 渲染「视频不存在」）
                return self._serve_file("detail.html")
            m = _DY_USER_PATH_RE.match(path)
            if m:
                sec = m.group("sec")
                line_nos = state.author_line_nos_by_sec_uid(sec)
                if not line_nos:
                    return self._page_miss()
                st = self._profile_state(state, sec, state.dataset().read(line_nos[0]))
                st["author"]["sec_uid"] = sec
                return self._serve_state_page("profile.html", st)
            if path == "/detail":
        ...
        if self.site == "xhs":
            ...
            if item_id is not None:
                if state.line_no_of(item_id) is None:
                    # 真站实证（live）：302 → /404?source=/404/sec_…&error_code=300031&error_msg=当前笔记暂时无法浏览
                ...
                return self._serve_state_page("detail.html", self._detail_state(state, item_id))
            ...
            if m:
                ...
                if not line_nos:
                    return self._page_miss()
                st = self._profile_state(state, uid, state.dataset().read(line_nos[0]))
                return self._serve_state_page("profile.html", st)
            return None
        ...
        if m:
            ...
            return self._serve_state_page("detail.html", st or {})
        ...
        if m:
            ...
            if not line_nos:
                return self._page_miss()
            st = self._profile_state(state, pid, state.dataset().read(line_nos[0]))
            return self._serve_state_page("profile.html", st)
        if path == "/detail":
            item_id = (params.get("id") or [None])[0]
            if item_id is not None:
